...
exports.EVENTS = [ "partBegin", "partEnd", "data", "end" ];
  ...
  while (parser.buffer) {
    switch (parser.state) {
      case S.NEW_PART:
        ...
        if (offset > 0) {
          return error(parser, "Malformed: data before the boundary");
        }
      ...
      case S.HEADER:
        ...
        try {
          parseHeaderString(parser.part.headers, headerString);
        } catch (ex) {
          error(parser, ex);
          return;
        }
