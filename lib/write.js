...
Writer.prototype.partBegin = function (partHeaders) {
  ...
  partString = CRLF + "--" + writer.boundary + CRLF;
  sys.puts(sys.inspect(partHeaders));
  Object.keys(partHeaders).forEach(function (key) {
    partString += key + ": " + partHeaders[key] + CRLF;
  });
  emit(writer, "onData",  partString + CRLF + CRLF);
...
Writer.prototype.partEnd = function () {
  var writer = this;
	...
	emit(writer, "onData", CRLF + "--" + writer.boundary + "--" );
	writer.state = S.PART_ENDED;
	writer.depth--;
	if (!writer.depth) emit(writer, "onEnd");
}
...
function writeHttpHeaders(writer) {
  ...
  emit(writer, "onData", headerString + CRLF);
}
